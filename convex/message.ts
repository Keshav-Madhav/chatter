...
export const create = mutation({
  ...
  handler : async (ctx, args) => {
    ...
    const membership = await ctx.db.query("conversation_members").withIndex("by_memberId_conversationId", q => q.eq("memberId", currentUser._id).eq("conversationId", args.conversationId)).unique();
