...
const validatePayload = async (req: Request): Promise<any> => {
  ...
  const clerkWekhookSecret = process.env.CLERK_WEBHOOK_SECRET!;
  if(!clerkWekhookSecret) throw new Error('Missing CLERK_WEBHOOK_SECRET');

  const webhook = new Webhook(clerkWekhookSecret)
...
const handler = httpAction(async (ctx, req) => {
  ...
  switch (event.type) {
    case 'user.created':
      ...
      if(user) console.log('Updating usr with new data: ', event.data.id)
